...
function withToolsPolicy(promptMessages, wsRow) {
  const parts = [wsRow ? ACTIVE_POLICY : GATE_POLICY, IMAGE_POLICY, SEARCH_POLICY];
  if (wsRow) {
    ...
    try {
      ...
      if (toolsOn && res.toolCalls?.length
        ...
        for (const call of res.toolCalls.slice(0, 2)) {
          ...
          if (!args?.prompt?.trim()) {
            ...
            try {
              ...
              const md = r.images.map((im) => `![generated image](${im.url})`).join('\n\n');
              mdImgs.push(md);
        ...
        if (gateCall) {
          ...
          loopMessages = withToolsPolicy(buildPrompt(conv, promptLeaf.id), wsRow);
