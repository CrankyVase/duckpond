<script>
  ...
  const HELP = {
    ...
    storage: 'Used space on the model storage filesystem, or app storage when no model directory is available, in GiB.',
    diskIo: 'Read and write throughput on the storage device shown here, in MiB/s.',
    ...
    ssdTemp: 'Storage NVMe temperature in °C, when its sensor is exposed.',
  };
