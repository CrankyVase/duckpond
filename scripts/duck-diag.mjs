import { DUCK, frameFrom } from '../web/src/lib/duck.js';
import { buildRects } from '../web/src/lib/pixel.js';
import { writeFileSync } from 'node:fs';
function svg(map, scale=14, grid=true){
  const w=map[0].length, h=map.length;
  const rects=buildRects(map, DUCK.palette).map(r=>`<rect x="${r.x}" y="${r.y}" width="${r.w}" height="1" fill="${r.c}"/>`).join('');
  let g='';
  if(grid){
    for(let x=0;x<=w;x++){g+=`<line x1="${x}" y1="0" x2="${x}" y2="${h}" stroke="${x%4===0?'#3a5':'#1e1e22'}" stroke-width="0.05"/>`;}
...
const items = (process.argv[3]||'stand').split(';').map(s=>{
  const d={}; s.split(',').forEach(kv=>{const[k,v]=kv.split(':'); if(k)d[k]=v==='true'?true:v;}); return [s,d];
});
let html=`<body style="background:#0f0f12;padding:12px">`+
  items.map(([l,d])=>`<div style="display:inline-block;margin:6px;color:#ccc;font:11px monospace;vertical-align:top">${svg(frameFrom(d,0))}<div>${l}</div></div>`).join('')+`</body>`;
writeFileSync(process.argv[2], html); console.log('ok');
